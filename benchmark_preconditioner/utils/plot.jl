...
plot(numspcs[inds], preconditioneddefaults[inds], "ko-.", label="Preconditioned - Default τ (1e-3)")
yscale(:log)
xscale(:log)
legend()
...
subplot(2, 2, 2)
inds = sortperm(numspcs)
plot(numspcs[inds], besttaus[inds], "o-", color="C1", label="Best")
...
plot(sparsitys[inds] * 100, preconditioneddefaults[inds] ./ denses[inds], "ko-.", label="Default τ (1e-3)")
